...
class Net(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Conv2d(3, 8, 3, padding=1) # output is (33 - f1) by (33 - f1) by m1
        self.pool = nn.MaxPool2d(2)
        self.pool_output_dims = int((33 - 3) / 2)
        self.conv2 = nn.Conv2d(8, 3, 3, padding=1) # output is (pool_output_dims - f2) ** 2 * m2
        self.conv2_output_size = ((self.pool_output_dims - 2) ** 2) * 12
        self.fc1 = nn.Linear(int(3 * 16 * 16), 25)
...
def train(trainloader, testloader, net, criterion, optimizer):
    ...
    while epoch < 75 and best_train < 100 and best_test < 100:  # loop over the dataset multiple times
        ...
        if best_test < test_accuracy:
            best_test = train_accuracy
            torch.save(net.state_dict(), "model_best_test_resnet.pt")
        if best_train < train_accuracy:
            best_train = test_accuracy
            torch.save(net.state_dict(), "model_best_train_resnet.pt")
