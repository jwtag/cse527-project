...
class Net(nn.Module):
    def __init__(self, acid_seq_length, num_drug_type_1, num_drug_type_2, num_drug_type_3):
        ...
        # define the networks used on each drug type
        ...
        self.drug_type_1_net = nn.Sequential(
            ...
            nn.Flatten(),
            nn.Linear(in_features=num_filters * (acid_seq_length - kernel_size + 1), out_features=num_drug_type_1)
        )
        self.drug_type_2_net = nn.Sequential(
            nn.Conv1d(in_channels=1, out_channels=num_filters, kernel_size=kernel_size),
            nn.ReLU(inplace=True),
            nn.Flatten(),
            nn.Linear(in_features=num_filters * (acid_seq_length - kernel_size + 1), out_features=num_drug_type_2)
        )
        self.drug_type_3_net = nn.Sequential(
            nn.Conv1d(in_channels=1, out_channels=num_filters, kernel_size=kernel_size),
            nn.ReLU(inplace=True),
            nn.Flatten(),
            nn.Linear(in_features=num_filters * (acid_seq_length - kernel_size + 1), out_features=num_drug_type_3)
        )
